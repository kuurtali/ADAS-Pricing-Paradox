...
# --- Paketler ---
required_pkgs <- c("dplyr", "statmod", "MASS", "ggplot2", "jsonlite", "corrplot", "gridExtra")
...
# Orijinal ham veriden ek degiskenleri al
...
# Dagitim Histogramlari
png("outputs/figures/eda_distributions.png", width = 1000, height = 400, res = 120)
par(mfrow = c(1, 2))
hist(data$Driver_Age, main = "Surucu Yasi Dagilimi", xlab = "Yas", col = "skyblue", border = "white")
...
###############################################################################
# BOLUM 2: FREKANS MODELI (Poisson GLM)
###############################################################################
...
###############################################################################
# BOLUM 4: RMSE TESTI & TUM VERIDE TAHMIN
###############################################################################
...
# Siddet RMSE sadece hasari olanlarda
...
# TUM veri icin tahmin yap (Power BI ve Paradox analizi icin)
...
###############################################################################
# BOLUM 5: DIAGNOSTIK GRAFIKLER
###############################################################################
png("outputs/figures/freq_residuals.png", width = 900, height = 400)
par(mfrow = c(1, 2), mar = c(4, 4, 3, 1))
plot(fitted(model_freq), residuals(model_freq, type = "pearson"), main = "Frequency Residuals vs Fitted", pch = 20, col = rgb(0, 0, 0, 0.15))
abline(h = 0, col = "red", lwd = 2)
qqnorm(residuals(model_freq, type = "pearson"), pch = 20, col = rgb(0, 0, 0, 0.15))
qqline(residuals(model_freq, type = "pearson"), col = "red", lwd = 2)
dev.off()

png("outputs/figures/sev_residuals.png", width = 900, height = 400)
par(mfrow = c(1, 2), mar = c(4, 4, 3, 1))
plot(fitted(model_sev), residuals(model_sev, type = "pearson"), main = "Severity Residuals vs Fitted", pch = 20, col = rgb(0, 0, 0, 0.15))
abline(h = 0, col = "red", lwd = 2)
qqnorm(residuals(model_sev, type = "pearson"), pch = 20, col = rgb(0, 0, 0, 0.15))
qqline(residuals(model_sev, type = "pearson"), col = "red", lwd = 2)
dev.off()

###############################################################################
# BOLUM 6: PARADOKS ANALIZI VE GGPLOT ILE GUVEN ARALIKLARI
###############################################################################
...
# ggplot2 ile ana paradoks grafikleri (Guven araliklari eklendi)
plot_freq <- ggplot(paradox, aes(x = paste("ADAS", Safety_Package_Level), y = Ort_Frekans, fill = Safety_Package_Level)) +
  ...
  labs(title = "Hasar Frekansi (+/-%95 GA)", x = NULL, y = "Ort. Tahmin Edilen Frekans") +
...
plot_sev <- ggplot(paradox, aes(x = paste("ADAS", Safety_Package_Level), y = Ort_Siddet, fill = Safety_Package_Level)) +
  ...
  scale_fill_manual(values = c("#2ecc71", "#f39c12", "#e74c3c")) +
  labs(title = "Hasar Siddeti (+/-%95 GA)", x = NULL, y = "Ort. Tahmin Edilen Siddet (TL)") +
  theme_minimal() + theme(legend.position = "none")
...
plot_prem <- ggplot(paradox, aes(x = paste("ADAS", Safety_Package_Level), y = Ort_Risk_Primi, fill = Safety_Package_Level)) +
  ...
  scale_fill_manual(values = c("#2ecc71", "#f39c12", "#e74c3c")) +
  labs(title = "Risk Primi Net Etki (+/-%95 GA)", x = NULL, y = "Ort. Risk Primi (TL)") +
  theme_minimal() + theme(legend.position = "none")
...
p_city <- ggplot(segment_city, aes(x = City, y = Ort_Risk_Primi, fill = Safety_Package_Level)) +
  ...
  scale_fill_manual(values = c("#2ecc71", "#f39c12", "#e74c3c"), labels = paste("ADAS", levels(data$Safety_Package_Level))) +
  labs(title = "Sehir Bazinda ADAS Risk Primi", x = "Sehir", y = "Ort. Risk Primi (TL)", fill = "ADAS Seviyesi") +
  theme_minimal(base_size = 13)
print(p_city)
dev.off()

###############################################################################
# BOLUM 7: ZENGINLESTIRILMIS CIKTI
###############################################################################
...
out_cols <- c("Policy_ID", "City", "Safety_Package_Level", "Exposure", "Claim_Count", "Claim_Amount",
              "Pred_Frequency", "Pred_Severity", "Risk_Premium", "Freq_vs_Baseline", "Sev_vs_Baseline", "Paradox_Flag")
